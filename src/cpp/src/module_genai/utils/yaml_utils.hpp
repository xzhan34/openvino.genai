...
ModuleDesc parse_module(const YAML::Node& node) {
    ...
    if (node["params"] && node["params"].IsMap()) {
        for (YAML::const_iterator it = node["params"].begin(); it != node["params"].end(); ++it) {
            module.params[it->first.as<std::string>()] = it->second.IsScalar() ? it->second.as<std::string>() : "[Complex Value]";
        }
    }
...
inline PipelineModuleDesc load_config(const std::string& cfg_path) {
