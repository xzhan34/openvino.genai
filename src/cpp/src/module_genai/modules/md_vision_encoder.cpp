...
void VisionEncoderModule::run() {
    prepare_inputs();
    if (this->inputs.find("thw") == this->inputs.end() || this->inputs["thw"].data == nullptr) {
        std::cerr << "VisionEncoderModule[" << module_desc->name << "]: 'thw' input not found" << std::endl;
        return;
    }
    if (this->inputs.find("resized_size") == this->inputs.end() || this->inputs["resized_size"].data == nullptr) {
        std::cerr << "VisionEncoderModule[" << module_desc->name << "]: 'resized_size' input not found" << std::endl;
        return;
    }
    std::cout << "Run: " << ModuleTypeConverter::toString(static_cast<ModuleType>(module_desc->type)) << "["
              << module_desc->name << "]" << std::endl;
    auto input_image = this->inputs["thw"].data.as<ov::Tensor>();
    ov::Tensor image_embedding;
    ov::Tensor video_embedding;
    EncodedImage encoded;
    encoded.resized_source_size.height = this->inputs["resized_size"].data.as<std::vector<int64_t>>()[0];
    encoded.resized_source_size.width = this->inputs["resized_size"].data.as<std::vector<int64_t>>()[1];
    encoded.resized_source = this->inputs["thw"].data.as<ov::Tensor>();
