...
TEST(WanVAEDecoderTest, BuildsAndRuns) {
    ...
    const size_t base_dim = static_cast<size_t>(cfg.decoder_base_dim);
    const size_t z_dim = static_cast<size_t>(cfg.z_dim);
    const size_t dim_first = static_cast<size_t>(cfg.decoder_base_dim * cfg.dim_mult.back());
    const size_t dim_last = static_cast<size_t>(cfg.decoder_base_dim * cfg.dim_mult.front());

    add_conv3d_weights(source, "post_quant_conv", z_dim, z_dim, 1, 1, 1);
    add_conv3d_weights(source, "decoder.conv_in", dim_first, z_dim, 3, 3, 3);

    add_residual_block(source, "decoder.mid_block.resnets.0", dim_first, dim_first);
    add_attention_block(source, "decoder.mid_block.attentions.0", dim_first);
    add_residual_block(source, "decoder.mid_block.resnets.1", dim_first, dim_first);

    add_residual_block(source, "decoder.up_blocks.0.resnets.0", dim_first, dim_first);
    add_residual_block(source, "decoder.up_blocks.0.resnets.1", dim_first, dim_first);
    add_conv2d_weights(source, "decoder.up_blocks.0.upsamplers.0.resample.1", dim_first / 2, dim_first, 3, 3);

    add_residual_block(source, "decoder.up_blocks.1.resnets.0", dim_last, dim_last);
    add_residual_block(source, "decoder.up_blocks.1.resnets.1", dim_last, dim_last);

    add_rms_gamma(source, "decoder.norm_out.gamma", dim_last, false);
    add_conv3d_weights(source, "decoder.conv_out", static_cast<size_t>(cfg.out_channels), dim_last, 3, 3, 3);

    auto model = ov::genai::modeling::models::create_wan_vae_decoder_model(cfg, source, finalizer);

    ov::Core core;
    auto compiled = core.compile_model(model, "CPU");
    auto request = compiled.create_infer_request();

    const ov::Shape input_shape{1, static_cast<size_t>(cfg.z_dim), 2, 2, 2};
    ov::Tensor input(ov::element::f32, input_shape);
    std::fill_n(input.data<float>(), input.get_size(), 0.0f);
    request.set_tensor("latents", input);
    request.infer();

    ov::Tensor output = request.get_tensor("sample");
    EXPECT_EQ(output.get_element_type(), ov::element::f32);
    EXPECT_EQ(output.get_shape(), (ov::Shape{1, static_cast<size_t>(cfg.out_channels), 2, 4, 4}));
