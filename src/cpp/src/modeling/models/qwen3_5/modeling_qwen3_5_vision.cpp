...
Qwen3_5VisionPatchEmbed::Qwen3_5VisionPatchEmbed(BuilderContext& ctx,
...
const Tensor& Qwen3_5VisionPatchEmbed::weight() const {
    if (!weight_param_) {
        OPENVINO_THROW("Qwen3_5VisionPatchEmbed weight parameter not registered");
    }
    return weight_param_->value();
}

const Tensor* Qwen3_5VisionPatchEmbed::bias() const {
    return (bias_param_ && bias_param_->is_bound()) ? &bias_param_->value() : nullptr;
}

Tensor Qwen3_5VisionPatchEmbed::forward(const Tensor& pixel_values) const {
...
Qwen3_5VisionAttention::Qwen3_5VisionAttention(BuilderContext& ctx,
                                               const std::string& name,
                                               const Qwen3_5VisionConfig& cfg,
                                               Module* parent)
    : Module(name, ctx, parent),
    ...
    if (hidden_size_ <= 0 || num_heads_ <= 0 || head_dim_ <= 0) {
        OPENVINO_THROW("Invalid Qwen3_5VisionAttention configuration");
    }
    if (hidden_size_ != num_heads_ * head_dim_) {
        OPENVINO_THROW("Qwen3_5VisionAttention hidden_size must equal num_heads * head_dim");
    }
    if (head_dim_ % 2 != 0) {
        OPENVINO_THROW("Qwen3_5VisionAttention head_dim must be even for rotary embeddings");
    }
...
const Tensor& Qwen3_5VisionAttention::qkv_weight() const {
    if (!qkv_weight_param_) {
        OPENVINO_THROW("Qwen3_5VisionAttention qkv weight parameter not registered");
    }
    return qkv_weight_param_->value();
}

const Tensor* Qwen3_5VisionAttention::qkv_bias() const {
    return (qkv_bias_param_ && qkv_bias_param_->is_bound()) ? &qkv_bias_param_->value() : nullptr;
}

const Tensor& Qwen3_5VisionAttention::proj_weight() const {
    if (!proj_weight_param_) {
        OPENVINO_THROW("Qwen3_5VisionAttention proj weight parameter not registered");
    }
    return proj_weight_param_->value();
}

const Tensor* Qwen3_5VisionAttention::proj_bias() const {
    return (proj_bias_param_ && proj_bias_param_->is_bound()) ? &proj_bias_param_->value() : nullptr;
}

Tensor Qwen3_5VisionAttention::apply_rotary(const Tensor& x,
                                            const Tensor& cos,
...
Qwen3_5VisionMLP::Qwen3_5VisionMLP(BuilderContext& ctx,
                                   const std::string& name,
                                   const Qwen3_5VisionConfig& cfg,
                                   Module* parent)
    : Module(name, ctx, parent) {
    if (!cfg.hidden_act.empty() && cfg.hidden_act != "gelu_pytorch_tanh") {
        OPENVINO_THROW("Unsupported Qwen3_5Vision MLP activation: ", cfg.hidden_act);
    }
...
const Tensor& Qwen3_5VisionMLP::fc1_weight() const {
    if (!fc1_weight_param_) {
        OPENVINO_THROW("Qwen3_5VisionMLP fc1 weight parameter not registered");
    }
    return fc1_weight_param_->value();
}

const Tensor* Qwen3_5VisionMLP::fc1_bias() const {
    return (fc1_bias_param_ && fc1_bias_param_->is_bound()) ? &fc1_bias_param_->value() : nullptr;
}

const Tensor& Qwen3_5VisionMLP::fc2_weight() const {
    if (!fc2_weight_param_) {
        OPENVINO_THROW("Qwen3_5VisionMLP fc2 weight parameter not registered");
    }
    return fc2_weight_param_->value();
}

const Tensor* Qwen3_5VisionMLP::fc2_bias() const {
    return (fc2_bias_param_ && fc2_bias_param_->is_bound()) ? &fc2_bias_param_->value() : nullptr;
}

Tensor Qwen3_5VisionMLP::forward(const Tensor& hidden_states) const {
...
Qwen3_5VisionBlock::Qwen3_5VisionBlock(BuilderContext& ctx,
                                       const std::string& name,
                                       const Qwen3_5VisionConfig& cfg,
                                       Module* parent)
    : Module(name, ctx, parent),
...
const Tensor& Qwen3_5VisionBlock::norm1_weight() const {
    if (!norm1_weight_param_) {
        OPENVINO_THROW("Qwen3_5VisionBlock norm1 weight parameter not registered");
    }
    return norm1_weight_param_->value();
}

const Tensor& Qwen3_5VisionBlock::norm1_bias() const {
    if (!norm1_bias_param_) {
        OPENVINO_THROW("Qwen3_5VisionBlock norm1 bias parameter not registered");
    }
    return norm1_bias_param_->value();
}

const Tensor& Qwen3_5VisionBlock::norm2_weight() const {
    if (!norm2_weight_param_) {
        OPENVINO_THROW("Qwen3_5VisionBlock norm2 weight parameter not registered");
    }
    return norm2_weight_param_->value();
}

const Tensor& Qwen3_5VisionBlock::norm2_bias() const {
    if (!norm2_bias_param_) {
        OPENVINO_THROW("Qwen3_5VisionBlock norm2 bias parameter not registered");
    }
...
Qwen3_5VisionPatchMerger::Qwen3_5VisionPatchMerger(BuilderContext& ctx,
                                                   const std::string& name,
                                                   const Qwen3_5VisionConfig& cfg,
...
const Tensor& Qwen3_5VisionPatchMerger::norm_weight() const {
    if (!norm_weight_param_) {
        OPENVINO_THROW("Qwen3_5VisionPatchMerger norm weight parameter not registered");
    }
    return norm_weight_param_->value();
}

const Tensor& Qwen3_5VisionPatchMerger::norm_bias() const {
    if (!norm_bias_param_) {
        OPENVINO_THROW("Qwen3_5VisionPatchMerger norm bias parameter not registered");
    }
    return norm_bias_param_->value();
}

const Tensor& Qwen3_5VisionPatchMerger::fc1_weight() const {
    if (!fc1_weight_param_) {
        OPENVINO_THROW("Qwen3_5VisionPatchMerger fc1 weight parameter not registered");
    }
    return fc1_weight_param_->value();
}

const Tensor* Qwen3_5VisionPatchMerger::fc1_bias() const {
    return (fc1_bias_param_ && fc1_bias_param_->is_bound()) ? &fc1_bias_param_->value() : nullptr;
}

const Tensor& Qwen3_5VisionPatchMerger::fc2_weight() const {
    if (!fc2_weight_param_) {
        OPENVINO_THROW("Qwen3_5VisionPatchMerger fc2 weight parameter not registered");
    }
    return fc2_weight_param_->value();
}

const Tensor* Qwen3_5VisionPatchMerger::fc2_bias() const {
    return (fc2_bias_param_ && fc2_bias_param_->is_bound()) ? &fc2_bias_param_->value() : nullptr;
}

Tensor Qwen3_5VisionPatchMerger::forward(const Tensor& hidden_states) const {
    Tensor x;
...
Qwen3_5VisionModel::Qwen3_5VisionModel(BuilderContext& ctx,
                                       const Qwen3_5VisionConfig& cfg,
                                       Module* parent)
    : Module(Qwen3_5ModuleNames::kVision, ctx, parent),
      cfg_(cfg),
    ...
    for (int32_t i = 0; i < cfg.depth; ++i) {
        blocks_.emplace_back(ctx, Qwen3_5ModuleNames::vision_block(i), cfg, this);
        register_module(Qwen3_5ModuleNames::vision_block(i), &blocks_.back());
    }

    deepstack_mergers_.reserve(deepstack_indexes_.size());
    for (size_t i = 0; i < deepstack_indexes_.size(); ++i) {
        deepstack_mergers_.emplace_back(ctx, Qwen3_5ModuleNames::deepstack_merger(static_cast<int32_t>(i)),
                                        cfg, true, this);
        register_module(Qwen3_5ModuleNames::deepstack_merger(static_cast<int32_t>(i)),
                        &deepstack_mergers_.back());
    }
}

Qwen3_5VisionOutput Qwen3_5VisionModel::forward(const Tensor& pixel_values,
...
Qwen3_5VisionPatchEmbed& Qwen3_5VisionModel::patch_embed() {
    return patch_embed_;
}

Qwen3_5VisionPatchMerger& Qwen3_5VisionModel::merger() {
    return merger_;
}
...
std::shared_ptr<ov::Model> create_qwen3_5_vision_model(
    ...
    auto pixel_values = ctx.parameter(Qwen3_5VisionIO::kPixelValues,
                                      ov::element::f32,
                                      ov::PartialShape{-1,
                                                       ...
                                                       cfg.vision.patch_size});
    auto grid_thw = ctx.parameter(Qwen3_5VisionIO::kGridThw,
                                  ov::element::i64,
                                  ov::PartialShape{-1, 3});
    auto pos_embeds = ctx.parameter(Qwen3_5VisionIO::kPosEmbeds,
                                    ov::element::f32,
                                    ov::PartialShape{-1, cfg.vision.hidden_size});
    auto rotary_cos = ctx.parameter(Qwen3_5VisionIO::kRotaryCos,
                                    ov::element::f32,
                                    ov::PartialShape{-1, cfg.vision.head_dim()});
    auto rotary_sin = ctx.parameter(Qwen3_5VisionIO::kRotarySin,
                                    ov::element::f32,
                                    ov::PartialShape{-1, cfg.vision.head_dim()});
    ...
    set_name(visual, Qwen3_5VisionIO::kVisualEmbeds);
    ...
    for (size_t i = 0; i < output.deepstack_embeds.size(); ++i) {
        ...
        std::string name = std::string(Qwen3_5VisionIO::kDeepstackEmbedsPrefix) + "." + std::to_string(i);
