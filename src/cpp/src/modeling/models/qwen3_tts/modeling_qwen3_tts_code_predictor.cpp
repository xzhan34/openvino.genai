...
Qwen3TTSCodePredictorForConditionalGeneration::Qwen3TTSCodePredictorForConditionalGeneration(
    ...
    const Qwen3TTSCodePredictorConfig& cfg,
    Module* parent)
    : Module("code_predictor", ctx, parent),
      cfg_(cfg),
      model_(ctx, cfg, this),
      codec_embeddings_(),
      lm_heads_() {
    // Create 15 codec embeddings (for layers 1-15)
    // Note: codec layer 0 is generated by Talker, so Code Predictor handles layers 1-15
    codec_embeddings_.reserve(15);
    for (int i = 0; i < 15; ++i) {
        codec_embeddings_.emplace_back(ctx, "codec_embeddings[" + std::to_string(i) + "]", this);
    }

    // Create 15 lm_heads (for predicting layers 1-15)
    lm_heads_.reserve(15);
    for (int i = 0; i < 15; ++i) {
        lm_heads_.emplace_back(ctx, "lm_heads[" + std::to_string(i) + "]", this);
    }
}
...
Tensor Qwen3TTSCodePredictorForConditionalGeneration::forward_no_cache(const Tensor& inputs_embeds,
    ...
    auto hidden_states = model_.forward_no_cache(inputs_embeds, position_ids);

    // Use appropriate lm_head for this step
    return lm_heads_[step].forward(hidden_states);
}
...
Tensor Qwen3TTSCodePredictorForConditionalGeneration::get_codec_embeds_sum(
    ...
    Tensor result = codec_embeddings_[0].forward(codec_ids_list[0]);
    for (size_t i = 1; i < codec_ids_list.size() && i < 15; ++i) {
        result = result + codec_embeddings_[i].forward(codec_ids_list[i]);
    }
    return result;
}
