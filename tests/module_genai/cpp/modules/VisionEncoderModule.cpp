...
protected:
    ...
    ov::AnyMap prepare_inputs() override {
        ov::AnyMap inputs;
        inputs["preprocessed_image"] = std::vector<ov::Tensor> {m_test_data.pixel_values};
        inputs["image_grid_thw"] = std::vector<ov::Tensor> {m_test_data.grid_thw};
        inputs["image_pos_embeds"] = std::vector<ov::Tensor> {m_test_data.pos_embeds};
        inputs["image_rotary_cos"] = std::vector<ov::Tensor> {m_test_data.rotary_cos};
        inputs["image_rotary_sin"] = std::vector<ov::Tensor> {m_test_data.rotary_sin};
        inputs["input_ids"]      = m_test_data.input_ids;
